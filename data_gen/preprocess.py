...
# def pre_normalization(data, zaxis=[0, 1], xaxis=[8, 4]):    #  UTD_MHAD  zaxis=[3,2]   xaxis=[8,4]
def pre_normalization(data, zaxis=[6, 3], xaxis=[1, 0]):      #  MSRA  zaxis=[6,3]   xaxis=[1,0]
# def pre_normalization(data, zaxis=[3, 2], xaxis=[8, 4]):
    ...
    for i_s, skeleton in enumerate(tqdm(s)):
        ...
        # main_body_center = skeleton[0][:, 2:3, :].copy()     #  UTD_MHAD
        main_body_center = skeleton[0][:, 3:4, :].copy()     #  MSRA
        for i_p, person in enumerate(skeleton):
            if person.sum() == 0:
                continue
    ...
    for i_s, skeleton in enumerate(tqdm(s)):
        if skeleton.sum() == 0:
            continue
        joint_bottom = skeleton[0, 0, zaxis[0]]       # zaxis=[0, 1], xaxis=[8, 4]
